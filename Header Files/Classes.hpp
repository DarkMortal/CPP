...
namespace Maths{
	class Complex{
		...
		public: Complex(){ x=0; y=0; }
			...
			float real(){ return x; }
			float img(){ return y; }
			void Initialze(float a,float b){ x=a; y=b; }
			...
			Complex operator /(Complex &e){
				float n;
				Complex a,b; a.Initialze(x,y);
				b=e.Conj();
				a=a*b;
				n=pow(e.MOD(),-2);
				a=a*n; return a;
			}
			Complex (Complex &a){ x=a.x; y=a.y; } 
	...
	class Vector{
		...
		void Initialze(float a,float b,float c){ x=a; y=b; z=c; } 
...
Complex Sqrt(Complex e){
	Complex x;
	float a=e.MOD()+e.real(); a=sqrt(a/2);
	...
	public: void getdata();
	  ...
	  void SearchBook(char x[]){
			  ifstream fl; int cnt=0;
				...
				fl.read((char *)this,sizeof(*this));
				while(!fl.eof()){ 
				    if(strcmp(x,title)==0||strcmp(x,ISBN)==0||strcmp(x,author)==0){ cnt++; disp();}
				    fl.read((char *)this,sizeof(*this));}
				if(cnt==0) cout<<"\nSorry, record doesn't exist!!!";
				else cout<<"\nThere are "<<cnt<<" records in total";
}}
  bool operator ==(BOOK &x){
  	if(x.price==price && strcmp(x.title,title)==0 && strcmp(x.ISBN,ISBN)==0)
  		return true;
  	else return false;
  }
  void DELETE(char a[]){
  	ifstream x; ofstream y;
	...
	cout<<"Enter the TITLE of the BOOK:-"; fflush(stdin); gets(title);
	cout<<"Enter the Author of the BOOK:-"; fflush(stdin); gets(author);
	cout<<"Enter ISBN Number:-"; cin>>ISBN; cout<<endl;
} void BOOK::disp(){
...
class Student{
    ...
    public:
	   void getdata(){
	   cout<<"\nEnter RollNO of STUDENT:-"; cin>>rn;
	   cout<<"Enter Name of STUDENT:-"; fflush(stdin); fflush(stdin); gets(name);
	   cout<<"Enter School of STUDENT:-"; fflush(stdin); fflush(stdin); gets(school);
	   cout<<"Enter the Prcentage:-"; cin>>prctg;}
	   void disp(){
  	...
  	if(x.rn==rn && strcmp(x.name,name)==0 && strcmp(x.school,school)==0 && x.prctg==prctg)
  		return true;
  	else return false;
  }
